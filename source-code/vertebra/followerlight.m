function followerlight(hobj)
	set(hobj,'HitTest','off'); % same as in Matt's example
	hax = ancestor(hobj,'axes'); % get the parent axes
	set(hax,'NextPlot','add'); % same as in Matt's example
	set(hax,'buttondownfcn', @buttondownfcn); % same as in Matt's example
	
    material(hax,'shiny') % or whatever reflectance you prefer
	axis(hax,'image')
	view(hax,3)
	grid(hax,'on')
	
    c = camlight('right');              % add light
    ...
    function rotationcallback(~,~,ax,oloc,oaz,oel)
        ...
        c = camlight(c,'right');        % adjust light
    end
